...
# ========= TELEGRAM TOKEN =========
# Có thể set env BOT_TOKEN trên Render, hoặc sửa trực tiếp ở đây
TOKEN = os.getenv("BOT_TOKEN", "8340989991:AAFbc5IiM5onGkvJDdzTrVzBgvseMrD-8xA")
...
def get_klines(symbol="BTCUSDT", interval="1h", limit=100):
    ...
    - BTCDOMUSDT: thử futures (fapi), nếu bị 418 thì báo lỗi rõ.
    - Các cặp ...USDT khác: ưu tiên futures, lỗi thì fallback về spot.
    - Còn lại: dùng spot luôn.
    """
    params = {"symbol": symbol, "interval": interval, "limit": limit}
    headers = {"User-Agent": "Mozilla/5.0"}

    # Case đặc biệt: BTCDOMUSDT
    ...
    # Các cặp ...USDT khác: ưu tiên futures
    if symbol.endswith("USDT"):
        try:
            ...
            # Có thể không có futures / lỗi → fallback về spot
            pass
...
def get_indicators(symbol, tf):
    ...
    # Vị trí giá hiện tại trong range 14 nến gần nhất
    range_pos_14 = None
...
if __name__ == "__main__":
    try:
        if TOKEN == "PUT_YOUR_TOKEN_HERE":
            raise RuntimeError(
                "Chưa cấu hình BOT_TOKEN. Set env BOT_TOKEN trên Render hoặc sửa trực tiếp TOKEN trong file bot.py."
            )

        logger.info("Starting bot with token prefix: %s***", TOKEN[:5])
        ...
        app.run_polling()
        logger.info("Bot stopped (run_polling returned).")
